	...
	public Board (EventHandler<MouseEvent> clicListener) {
		...
		for (int ligne = 0; ligne < nbLig; ligne++){
			for (int col = 0; col < nbCol; col++) {
				...
				///// Si une pièe doit se trouver sur cette case /////

				// création de la pièe uniquement si doit être sur cette case
				piece = GuiFactory.createPiece(col, ligne);

				if (piece != null) {

					// ajout d'un écouteur de souris
					// si la pièe est sélectionnée, elle sera supprimé de son emplacement actuel
					// et repositionnée sur une autre case
					piece.setOnMouseClicked(clicListener);

					// gestion de la taille et position de la pièe (au centre du carré)
					...
					// Ajout de la pièe sur le carré noir
	...
	public void actionOnGui(InputViewData<Integer> dataToRefreshView) {
		...
		if (dataToRefreshView != null) {
			...
			if (dataToRefreshView.toMovePieceIndex != -1 && dataToRefreshView.targetSquareIndex != -1) {
				...
				// clear la case d'origine de la pièe déplacée
				if (toMovePiece != null) {
			...
			////////////////////////////////////////////////////
			// l'éventuelle pièe intermédiaire est supprimée
			////////////////////////////////////////////////////
			if (dataToRefreshView.capturedPieceIndex != -1) {
				// clear la case d'origine de la pièe supprimée
