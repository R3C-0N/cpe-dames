 ...
 *    
 * La view et le model ne gêrant pas les coordonnées des cases de la même maniêre
...
public class Controller implements Mediator, BoardGame<Integer>, EventHandler<MouseEvent>  {
	...
	// Cette valeur est MAJ chaque fois que l'utilisateur clique sur une pièe
	...
	private void checkersSquareGuiHandle(MouseEvent mouseEvent) {
		...
		// il n'y a plus de pièe à déplacer
		this.setToMovePieceIndex(-1);
	...
	public OutputModelData<Integer> moveCapturePromote(Integer toMovePieceIndex, Integer targetSquareIndex) {
		...
		OutputModelData<Coord> moved = this.model.moveCapturePromote(pieceCoord, squareCoord);

		if (moved.isMoveDone){
			Integer capturedPieceIndex = this.transformCoordToIndex(moved.capturedPieceCoord);
			Integer promotedPieceIndex = this.transformCoordToIndex(moved.promotedPieceCoord);
			InputViewData<Integer> dataToRefreshView = new InputViewData<>(
					toMovePieceIndex,
					targetSquareIndex,
					capturedPieceIndex,
					promotedPieceIndex,
					moved.promotedPieceColor
			);
			this.view.actionOnGui(dataToRefreshView);
		}
